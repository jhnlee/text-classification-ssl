...
def train(
    args, train_dataset, model: BertForMaskedLM, tokenizer: BertTokenizer
) -> Tuple[int, float]:
    ...
    def collate(data: List[torch.Tensor]):
        sentences, _ = list(zip(*data))
        if tokenizer._pad_token is None:
            return pad_sequence(sentences, batch_first=True)
        # Do not use labels in Masked Language Model
    ...
    # Train!
    logger.info("***** Running Masked Language Model training *****")
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Do not use stopwords while mlm finetuning
            inputs, mask_labels = mask_tokens(batch, tokenizer)

            inputs = inputs.to(args.device)
            mask_labels = mask_labels.to(args.device)
            model.train()
            outputs = model(input_ids=inputs, masked_lm_labels=mask_labels)
            loss = outputs[0]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    checkpoint_prefix = "mlm_checkpoint"
...
def evaluate(args, model: BertForMaskedLM, tokenizer: BertTokenizer, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            loss = model(input_ids=inputs, masked_lm_labels=mask_labels)
            loss = loss[0]
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = TextDataset(tokenizer, file_path=args.train_data_file)
